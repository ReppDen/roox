...
import ru.repp.den.exception.ForbiddenException;
...
public class PartnerMappingServiceImpl implements PartnerMappingService{
    ...
    @Override
    public boolean createAvatarForMapping(String id, Long mapId, MultipartFile file) {
        validateRequest(id);
    ...
    private void validateRequest(String customerId) {
        if (!us.isAllowedToProcess(customerId)) {
            throw new ForbiddenException("You are not allowed to watch partner mappings of other customer");
        }
    }
}
